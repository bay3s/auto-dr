...
class MultiprocessingVecEnv(VecEnv):
    ...
    def step(self, actions: np.ndarray) -> VecEnvStepReturn:
        ...
        Args:
          actions (np.ndarray): Actions for each of the environments.
    ...
    def update_tasks_async(self, tasks: List) -> None:
        """
        Sends a `set_task` request to each of the environments.

        Returns:
            None
        """
        for remote, task in zip(self.remotes, tasks):
            remote.send(("update_task", task))
